...
== 예제     : var objChart = new XlbBarChart01("xlbChart");
=====================================================================
*/
var XlbBarChart01 = function(divId){

    //속성정의 
    this.tmpltCode  = "XBAC01"; //템플릿코드
    ...
    **********************************************************/
    this.cvrtListToChartData = function(dataSet, colPropInfo){
        ...
        for(var i=0; i < listData.length; i++){
            ...
            obj.xlbchart_tooltip = "";
            if(!fIsNone(colPropInfo.param3))
    ...
    **********************************************************/
    this.drawScale = function(data){
        if(this.viewScroll) {
            ...
            var yLength = d3.max(data, function(d) {return String(d.xlbchart_value).length;});
        ...
        .nice()
        .range([this.height - this.marginBottom, this.marginTop]); //높이
        ...
        function xAxis(g) {
            return g
                .attr(
                    'transform',
                    'translate(0,' + (vHeight - vMargin_bottom) + ')'
                )
        ...
        .enter()
        .append('rect') //차트 타입
        // .attr('fill', this.fillColor) // 막대 색상
        .attr('fill',function(d){return vFillColor(d.xlbchart_value);})
        .attr('x', function(d) {
            return x(d.xlbchart_name);
        })
        // .transition()    //애니메이션 생략
        // .duration(1000)
        // .delay(function(d,i){
        ...
        // })
        .attr('y', function(d) {
            return y(d.xlbchart_value);
        })
        .attr('height', function(d) {
            return y(0) - y(d.xlbchart_value);
        })
